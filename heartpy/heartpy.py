...
include an index column?'
    ...
    if interp_clipping:
        print("Interpolate clipping is executed")
        if clipping_scale:
    ...
    if hampel_correct: # pragma: no cover
        print("hampel correction is executed")
    ...
    if bl_val < 0:
        #print(f'hrdata before bl_val added: {hrdata[0]}')
        hrdata = hrdata + abs(bl_val)
        #print(f'specific bl_val (which will be added to hrdata): {abs(bl_val)}')
        #print(f'hrdata with bl_val added: {hrdata[0]}')

    working_data['hr'] = hrdata
    #print(f'working_data[hr]: {working_data["hr"]}')
...
use either \'iqr\' or \'z-score\''
    ...
    print(f'hrdata in process_segmentwise at beginning: {hrdata[0]}')
    ...
    if mode == 'full':
        # i indicating start-index from sequence
        ## ii end-index from sequence
        for i, ii in slice_indices:
            #print(f'i: {i}')
            #print(f'ii: {ii}')
            try:
                #print(f'hrdata handed over to process function [i:ii]: {hrdata[0:1000]}')
                working_data, measures = process(hrdata[i:ii], sample_rate, **kwargs)
                for k in measures.keys():
                    #print(f'measures.keys: {measures.keys()}')
