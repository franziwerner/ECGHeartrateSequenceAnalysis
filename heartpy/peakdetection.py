...
def detect_peaks(hrdata, rol_mean, ma_perc, sample_rate, update_dict=True, working_data={}):
    ...
    #working_data['best'] = 50
    # funktioniert nur hardgecoded (nicht, wenn erste Zeile fehlt)
    #print('best ma_perc in working_data:' + str(working_data['best']))
...
def fit_peaks(hrdata, rol_mean, sample_rate, bpmmin=40, bpmmax=180, working_data={}):
    ...
    if len(valid_ma) > 0:
        ...
        ##working_data = detect_peaks(hrdata, rol_mean, 5,
        ##                            sample_rate, update_dict=True, working_data=working_data)
        #print('best ma_perc in working_data:' + str(min(valid_ma, key=lambda t: t[0])[1]))
        return working_data
    else:
